...
cnvPlot <- function(cnv, plot_title="", extend=1000) {
  ...
  # add adjusted position in bases
  # (spread probes with same PosKb slightly for visualisation)
  ...
  #
  # calclate approximate exon locations/boundaries (for visualisation only)
  #
  exons <- tibble(name = character(), start = integer(), end = integer())
  slop <- diff(xlim) * 0.01
  for (x in unique(cnv$Exon)) {
    exonStart <- min(cnv[cnv$Exon == x, "PosEst"])
    exonEnd <- max(cnv[cnv$Exon == x, "PosEst"])
    exonName <- x
    # spread slightly if single probe for better visualisation
    # if (exonStart == exonEnd) {
      exonStart <- exonStart - ifelse(nrow(exons)>0, min(exonStart - exons[nrow(exons),"end"], slop), slop)
      exonEnd <- exonEnd + slop
    # }
    exons <- exons %>% add_row(tibble_row(name = as.character(exonName), start = exonStart, end = exonEnd))
  }
  ...
  if (nrow(exons) >= 1) {
    ...
    wig <- 0.1
    # # plot gene
    lines(x = c(min(cnv$PosKb)*1000, max(cnv$PosKb)*1000), y = rep(lev + wig, 2))
    # # gene name
    text(x = exons$end, y = c(lev + (3 * wig), lev - wig), labels = exons$name, pos = 4, cex = 0.5)
    # plot exons
    for (e in 1:nrow(exons)) {
      # exon ticks
      vertical_offset <- 2 * wig * (e %% 2)
      col = ifelse(e %% 2 == 0, 'black', 'red')
      rect(exons[e,"start"], lev - wig + vertical_offset, exons[e,"end"], lev + wig + vertical_offset,
        col = col, border = NA)
    }
  }
  # add CNV overlay to exon
